...
public class OTLPTracesRestAction extends BaseRestHandler {
    @Override
    ...
    protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
        if (request.hasContent()) {
            var transportRequest = new OTLPTracesTransportAction.TracesRequest(request.content().retain());
            return channel -> client.execute(
                OTLPTracesTransportAction.TYPE,
                transportRequest,
                ActionListener.releaseBefore(request.content(), new RestResponseListener<>(channel) {
                    @Override
                    public RestResponse buildResponse(OTLPTracesTransportAction.TracesResponse r) {
        ...
        return channel -> channel.sendResponse(
            new RestResponse(
                ...
                new BytesArray(ExportTraceServiceResponse.newBuilder().build().toByteArray())
            )
        );
    }
}
